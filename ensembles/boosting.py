...
from .utils import ConvergenceHistory, whether_to_stop, rmsle, rmse
...
class GradientBoostingMSE:
    ...
    def fit(
        ...
        if (X_val is not None and y_val is not None) or trace:
            history = ConvergenceHistory({"train": [], "val": [], "time": []})
        else:
            history = None

        y_pred = np.full(y.shape, self.const_prediction)
        start = time.time()
        for tree in self.forest:
            y_tr = y - y_pred
            tree.fit(X, y_tr)
            self.n_trained_estimators += 1
            ...
            if history is not None:
                ...
                if X_val is not None and y_val is not None:
                    ...
                    val_loss = rmse(y_val, val_predictions)
                ...
                if patience is not None and whether_to_stop(history, patience):
                    self.forest = self.forest[:self.n_trained_estimators]
                    break
    ...
    def predict(self, X: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
        ...
        y_pred = self.const_prediction
        for i in range(self.n_trained_estimators):
            y_pred += self.forest[i].predict(X) * self.learning_rate
        return y_pred
